...
struct casio_task* find_casio_task_list(struct casio_rq* rq, struct task_struct* p){
	...
	struct casio_task* casio_task = NULL;
	if (rq && p){
		list_for_each_safe(ptr, &rq->casio_list_head){
                        casio_task = list_entry(ptr, struct casio_task, casio_list_node);
                        if (casio_task){
...
#ifdef CONFIG_SMP
static unsigned long load_balance_casio(struct rq* this_rq, int this_cpu, struct rq* busiest, unsigned long max_load_move, struct sched_domain* sd, enum cpu_idle_type idle, int* all_pinned, int* this_best_prio)
{
	return 0;
}

static int move_one_task_casio(struct rq* this_rq, int this_cpu, struct rq* busiest, struct sched_domain* sd, enum cpu_idle_type idle)
{
	return 0;
}
#endif
